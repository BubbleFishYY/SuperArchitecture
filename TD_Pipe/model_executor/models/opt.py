...
from TD_Pipe.model_executor.layers.attention import PagedAttention
...
class OPTDecoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.parallel_config.start, self.parallel_config.end + 1):
            self.layers.add_module(f"{i}", OPTDecoderLayer(config, linear_method))
...
class OPTModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.decoder = OPTDecoder(config, parallel_config, linear_method)
...
class OPTForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.model = OPTModel(config, parallel_config, linear_method)
