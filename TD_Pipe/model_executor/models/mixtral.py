...
class MixtralModel(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.parallel_config.start, self.parallel_config.end+1):
            self.layers.add_module(f"{i}",MixtralDecoderLayer(config, linear_method=linear_method))
...
class MixtralForCausalLM(nn.Module):
    def __init__(
        ...
        self.model = MixtralModel(config, parallel_config, linear_method)
